...
run:
	mkdir -p out-docker
	docker run -it -v $(PWD)/out-docker:/out $(tag) dt-logic-demo

run-with-mounted-src:
	mkdir -p out-docker
	docker run -it -v $(PWD)/src:/library/src:ro -v $(PWD)/out-docker:/out $(tag) dt-logic-demo
...
upload: # v3
	...
	dts build_utils check-need-upload --package dt-logic-ente make upload-do
